...
class IsriEnv(gym.Env):
    def __init__(self, env_config: Dict):
        ...
        self.observation_space = self.get_obs_space(env_config['obs_space'])
    ...
    def reset(self, *, seed=None, options=None):
        self.genome = []
        self.steps = 0
        random_index = random.choice(list(range(0, len(self.dataset['Jobdata']))))
        self.selected_index = random_index
        self.jobdata = self.dataset['Jobdata'][random_index]
        self.curr_target_values = self.dataset['GAFitness'][random_index]
        self.unplanned_jobs_sorted = sorted(self.jobdata, key=lambda x: self.jobdata[x]['due_date'])
        self.input_sequence = np.array(np.array([self.jobdata[job]['times'] + [self.jobdata[job]['due_date']]
                                 for job in self.unplanned_jobs_sorted])) / self.conv_speed
    ...
    def _read_config(self, env_config: Dict):
        ...
        self.use_heuristic = env_config['use_heuristic']
    ...
    def _get_reward_sparse(self):
        if self.steps == len(self.jobdata):
            ...
            if self.curr_goal == 'workload':
                ...
                if diffsum_difference > 0:
                    diffsum_difference = 0
                if tardiness_difference > 0:
                    tardiness_difference = 0
                return (diffsum_difference + tardiness_difference) * 100
        else:
    ...
    def _get_info(self):
        return {'deadline_gap': self.deadline_gap, 
                'workload_gap': self.workload_gap}
    ...
    def _update_genome(self, job):
        ...
        if self.use_heuristic:
            ...
            # Hänge den Job einfach hinten an
            self.genome.append(job)
    ...
    def get_obs_space(self, obs_space):
        if obs_space == 'simple':
            ...
            obs_space = gym.spaces.Box(-np.inf, np.inf, shape=obs_shape)
        elif obs_space == 'full':
            ...
            obs_shape = (1, obs_shape_len)
            obs_space = gym.spaces.Box(-np.inf, np.inf, shape=obs_shape)
        elif obs_space == 'transformer':
            obs_space = gym.spaces.Dict({
                'input_sequence': gym.spaces.Box(-np.inf, np.inf, shape=(20, self.features_per_job)),
                'tgt_sequence': gym.spaces.Box(-np.inf, np.inf, shape=(20, self.features_per_job))
            })
        elif obs_space == 'lstm':
            obs_space = gym.spaces.Dict({
                'input_sequence': gym.spaces.Box(-np.inf, np.inf, shape=(self.last_n, self.features_per_job)),
                'tgt_sequence': gym.spaces.Box(-np.inf, np.inf, shape=(self.next_n, self.features_per_job))
            })
    ...
    def make_obs(self):
        ...
        elif self.obs_type == 'transformer':
            return self._make_obs_transformer()
        elif self.obs_type == 'lstm':
            return self._make_obs_lstm()
    ...
    def _make_obs_transformer(self):
        # Transformer Architektur bekommt immer die gesamte Sequenz zum Encoden und die bisherige Sequenz für den Decoder
        if len(self.genome) > 0:
            tgt_sequence = np.array(np.array([self.jobdata[job]['times'] + [self.jobdata[job]['due_date']]
                                    for job in self.genome])) / self.conv_speed
        else:
            tgt_sequence = np.ones((1, self.features_per_job)) * -2
        tgt_sequence = pad_array(tgt_sequence, padding_size=20, padding_value=-1, dim=0)
        obs = {'input_sequence': self.input_sequence, 'tgt_sequence': tgt_sequence}
        return obs
    
    def _make_obs_lstm(self):
        # Extract times for the last self.last_n entries of planned_jobs
        last_n_array = np.ones((self.last_n, self.features_per_job)) * -1
        size_last_n = min((len(self.genome), self.last_n))
        if size_last_n > 0:
        ...
        obs = {'input_sequence': last_n_array, 'tgt_sequence': next_n_array}
        return obs
