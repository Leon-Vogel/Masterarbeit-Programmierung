...
MJD_PATH = "C:/Users/lukas/Documents/SUPPORT/Isringhausen/Daten/mtm_info.csv"
MULTIPROCESSING = True
PREPROCESSED_FILES_PATH = "./isri_optimizer/instances/"
...
GA_PARAMS = {
            ...
            "conveyor_speed": 208,
            "ma_window_size": 4,
            "n_evals": 8000,  # number of fitness calculations,
            "seed": 123,
...
def preprocess_data(params):
    ...
    jobs = find_next_jobs_by_due_date(GA_PARAMS['n_jobs'] * 2, 1, jobdata)
    jobs_a = jobs[::2]
    jobs_b = jobs[1::2]
    jobdata_a = {job: jobdata[job] for job in jobs_a}
    jobdata_b = {job: jobdata[job] for job in jobs_b}

    with open("./isri_instances/" + file.split(".")[0] + "_a.pkl", "wb") as outfile:
        pickle.dump(jobdata_a, outfile)

    with open("./isri_instances/" + file.split(".")[0] + "_b.pkl", "wb") as outfile:
...
if __name__ == '__main__':
    ...
    with open("IsriDataset.pkl", "wb") as outfile:
