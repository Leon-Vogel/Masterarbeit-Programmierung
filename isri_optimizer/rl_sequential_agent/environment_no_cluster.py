...
class IsriEnv_no_cluster(gym.Env):
    ...
    def _read_config(self, env_config: Dict):
        ...
        self.next_n = env_config["n_classes"]
    ...
    def get_obs_space(self, obs_space):
        if obs_space == 'classes':
            obs_shape = (1, self.next_n * 13 + self.last_n * 13 + 1) # Plus 1 für Step
    ...
    def _make_obs_classes(self):
        ...
        """
         # next n jobs by due date
        next_n_array = np.zeros((self.next_n, self.features_per_job)) * -1
        ...
        if size_next_n > 0:
            # Next N Jobs Data ( MTM + Deadline)
            next_n_times[:, :self.n_machines] /= self.conv_speed
            next_n_times[:, -1] = (next_n_times[:, -1] - self.conv_speed * self.steps) / (self.conv_speed*100)
            next_n_array[:size_next_n, :] = next_n_times
        ...
        obs_flat = np.concatenate([next_n_array.flatten(), last_n_array.flatten(), progress])
        return obs_flat