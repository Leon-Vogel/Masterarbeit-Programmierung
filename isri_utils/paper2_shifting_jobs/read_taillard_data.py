...
def get_taillard(dataset):
    with open(os.path.join(path_definitions.TAILLARD_DATA_ISRI_PAPER2_SHIFTING_JOBS, dataset), 'r') as file:
        contents = file.read()
    rows = contents.split("\n")
...
def get_taillard_with_uncert_proc_time(dataset="ta001"):
    filename = os.path.join(path_definitions.TAILLARD_DATA_ISRI_PAPER2_SHIFTING_JOBS, f"{dataset}_uncert.pkl")
...
if __name__ == "__main__":
    default_instances = os.listdir(path_definitions.TAILLARD_DATA_ISRI_PAPER2_SHIFTING_JOBS)
