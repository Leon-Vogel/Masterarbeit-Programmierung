...
# From: https://github.com/muhanzhang/D-VAE/blob/master/
'''
...
class DVAE(nn.Module):
    def __init__(self, max_n, nvt, START_TYPE, END_TYPE, hs=501,
                 nz=56, bidirectional=False, vid=True, nvf=6,
                 START_FEATS=[-1, -1, -1, -1, -1, -1, -1], nef=3):
        super(DVAE, self).__init__()
        self.max_n = max_n  # maximum number of vertices
        self.nvt = nvt  # number of vertex types
        self.nef = nef # number of edge features
        self.START_TYPE = START_TYPE
        self.END_TYPE = END_TYPE
        self.hs = hs  # hidden state size of each vertex
        self.nz = nz  # size of latent representation z
        ...
        self.vid = vid
        self.device = None
        self.nvf = nvf # number of vertex features
        self.START_FEATS = START_FEATS
        ...
        # 0. encoding-related
        self.grue_forward = nn.GRUCell(nvt + nvf, hs)  # encoder GRU
        self.grue_backward = nn.GRUCell(nvt + nvf, hs)  # backward encoder GRU
        self.fc1 = nn.Linear(self.gs, nz)  # latent mean
        self.fc2 = nn.Linear(self.gs, nz)  # latent logvar
            
        # 1. decoding-related
        self.grud = nn.GRUCell(nvt + nvf, hs)  # decoder GRU
        self.fc3 = nn.Linear(nz, hs)  # from latent z to initial hidden state h0
        self.add_vertex = nn.Sequential(
                nn.Linear(hs, hs * 2),
                nn.ReLU(),
        ...
        self.mapper_forward = nn.Sequential(
                nn.Linear(self.vs + self.nef, hs, bias=True),
                )  # disable bias to ensure padded zeros also mapped to zeros - no padding in our implementation (Lukas)
        ...
        # Added by me
        self.feature_decoding = nn.Sequential(
            nn.Linear(self.hs, nz),
            nn.ReLU(),
            nn.Linear(self.nz, nz),
            nn.ReLU(),
            nn.Linear(nz, nvf)
        )
    ...
    def _propagate_to_alternative(self, G, v, propagator, H=None, reverse=False):
        # propagate messages to vertex index v for all graphs in G
        # return the new messages (states) at v
        G = [g for g in G if g.vcount() > v]
        ...
        if H is not None:
            ...
            H = H[idx]
        v_types = [g.vs[v]['type'] for g in G]
        v_feats = [g.vs[v]['features'] for g in G]
        v_x = [v_types[idx] + v_feats[idx] for idx, g in enumerate(G)]
        X = torch.tensor(v_x, dtype=torch.float32, device=device)
        if reverse:
            H_name = 'H_backward'  # name of the hidden states attribute
            H_pred = [[g.vs[x][H_name] for x in g.successors(v)] for g in G]
            if self.vid:
            ...
            for g in G:
                ...
                for pred in g.predecessors(v):
                    hidden_pred = g.vs[pred][H_name]
                    edge_pred = g.es().find(_between=((pred, ), (v, )))['edge_type']
                    one_hot_edge = self._one_hot(edge_pred, self.nef)
                    h_pred.append(torch.cat((hidden_pred, one_hot_edge), 1))
                H_pred.append(h_pred)
            if self.vid:
                vids = [self._one_hot(g.predecessors(v), self.max_n) for g in G]
            gate, mapper = self.gate_forward, self.mapper_forward
        if self.vid:
        ...
        if H is None:
            ...
            if max_n_pred == 0:
                ...
                H_pred = [torch.cat(h_pred + 
                            [self._get_zeros(max_n_pred - len(h_pred), self.vs + self.nef)], 0).unsqueeze(0) 
                            for h_pred in H_pred]  # pad all to same length
                H_pred = torch.cat(H_pred, 0)  # batch * max_n_pred * vs
                H = self._gated(H_pred, gate, mapper).sum(1)  # batch * hs
        Hv = propagator(X, H)
    ...
    def _propagate_from(self, G, v, propagator, H0=None, reverse=False):
        ...
        Hv = self._propagate_to_alternative(G, v, propagator, H0, reverse=reverse)  # the initial vertex
        for v_ in prop_order[1:]:
            self._propagate_to_alternative(G, v_, propagator, reverse=reverse)
        return Hv

    def _update_v(self, G, v, H0=None):
        # perform a forward propagation step at v when decoding to update v's state
        self._propagate_to_alternative(G, v, self.grud, H0, reverse=False)
        return
    ...
    def _get_node_state(self, G, node, decode=True):
        Hg = []
        for g in G:
            hg = g.vs[node]['H_forward']
            Hg.append(hg)
        Hg = torch.cat(Hg, 0)
        return Hg
    ...
    def _get_edge_scores_alternative(self, Hvi, H, H0):
        return self.edge_scores(torch.cat([Hvi, H], -1))
    ...
    def decode_alternative(self, G, mu, logvar):
        device = self.get_device()
        z = self.reparameterize(mu, logvar)
        H0 = self.tanh(self.fc3(z))  # or relu activation, similar performance
        G[0].vs()[0]['features'] = [-1] * self.nvf
        self._update_v(G, 0, H0)
        n_nodes = max([g.vcount() for g in G])
        features_decoded = []
        for v_true in range(1, n_nodes):
            # Get Features of all predecessors:
            Hp = [self._propagate_to_alternative(G, pred, propagator=self.grud) for pred in G[0].predecessors(v_true)] # Kein BATCHING möglich!
            Hp = torch.cat(Hp, dim=0)
            Hp = torch.mean(Hp, dim=0).reshape(1, self.hs)
            vfe = self.feature_decoding(Hp).reshape(1, self.nvf)
            G[0].vs()[v_true]['features'] = vfe.detach().cpu().numpy().tolist()[0]
            features_decoded.append(vfe)
    ...
    def loss(self, mu, logvar, G_true, beta=0.005):
        ...
        for v_true in range(1, self.max_n):
            ...
            vll = self.logsoftmax1(type_scores)[np.arange(len(G)), true_types].sum()# TODO: KEIN Softmax
            res = res + vll
            for i, g in enumerate(G):
    ...
    def loss_alternative(self, mu, logvar, G_true, beta=0.005, n_nodes=None):
        # compute the loss of decoding mu and logvar to true graphs using teacher forcing
        # ensure when computing the loss of step i, steps 0 to i-1 are correct
        device = self.get_device()
        z = self.reparameterize(mu, logvar)
        ...
        G = [igraph.Graph(directed=True) for _ in range(len(z))]
        for g in G:
            g.add_vertex(type=self.START_TYPE, features=self.START_FEATS)
        self._update_v(G, 0, H0)
        res = 0  # log likelihood
        feature_error = 0
        for v_true in range(1, n_nodes):
            ...
            # use start type to denote padding vertices since start type only appears for vertex 0 
            # and will never be a true type for later vertices, thus it's free to use
            true_type = [g_true.vs[v_true]['type'] if v_true < g_true.vcount() 
                          else self.START_TYPE for g_true in G_true]
            true_features = [g_true.vs[v_true]['features'] if v_true < g_true.vcount() 
                          else self.START_TYPE for g_true in G_true]
            Hg = self._get_graph_state(G, decode=True)
            type_scores = self.add_vertex(Hg)
            # vertex error
            vll = self.logsoftmax1(type_scores)[np.arange(len(G)), true_type].sum()# TODO: KEIN Softmax
            vfe = self.feature_decoding(Hg)
            feature_error += self.feature_loss(torch.tensor(true_features, dtype=torch.float32, device=device), vfe)
            # res = res + vll # Vertex type is not important in our use case

            # Teacher forcing
            for i, g in enumerate(G):
                if true_type[i] != self.START_TYPE: 
                    g.add_vertex(type=true_type[i], features=true_features[i])
            self._update_v(G, v_true)
            ...
            for vi in range(v_true-1, -1, -1):
                Hvi = self._get_vertex_state(G, vi)
                H = self._get_vertex_state(G, v_true)
                ei_score = self._get_edge_scores_alternative(Hvi, H, H0)
                edge_scores.append(ei_score)
                true_scores.append(self._one_hot(g_true.es[0]["edge_type"], 3))
                for i, g in enumerate(G):
                    if vi in true_edges[i]:
                        g.add_edge(vi, v_true)
                self._update_v(G, v_true)
            edge_scores = torch.cat(edge_scores[::-1], 1).to(device)
            true_scores = torch.cat(true_scores[::-1], 1).to(device)

            # edges log-likelihood
            ell = - F.cross_entropy(edge_scores, true_scores, reduction='mean') 
            res = res + ell

        res = -res  # convert likelihood to loss
        kld = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        res = res + feature_error
        return res + beta*kld, res, kld

    def loss_features(self, mu, logvar, G, return_decoding=False):
        device = self.get_device()
        z = self.reparameterize(mu, logvar)
        H0 = self.tanh(self.fc3(z))  # or relu activation, similar performance
        self._update_v(G, 0, H0)
        n_nodes = max([g.vcount() for g in G])
        loss = 0
        decodings = [H0[idx, :] for idx in range(len(G))]
        for v_true in range(1, n_nodes):
            v_features = [g_true.vs[v_true]['features'] if v_true < g_true.vcount() 
                          else self.START_FEATS for g_true in G]
            # Get Features of all predecessors:
            Hp = [self._propagate_to_alternative(G, pred, propagator=self.grud) for pred in G[0].predecessors(v_true)] # Kein BATCHING möglich!
            Hp = torch.cat(Hp, dim=0)
            Hp = torch.mean(Hp, dim=0).reshape(1, self.hs)
            vfe = self.feature_decoding(Hp).reshape(1, self.nvf)
            for idx in range(vfe.shape[0]):
                loss += self.feature_loss(torch.tensor(v_features[idx], device=device, dtype=torch.float32), vfe[idx, :])
                decodings.append(vfe[idx, :])
        return loss
